...
export const Navbar=()=>{
    ...
    return <header className={`fixed top-0 left-0 right-0 transition-all duration-500 ${isScrolled ? "glass-strong py-1": "bg-transparent py-5"}  z-50`}>
        <nav className="container mx-auto py-6 flex items-center justify-between">
            <a href="#" className="text-xl font-bold tracking-tight hover:text-primary">
                SAA<span className="text-primary ">.</span>
            </a>
         ...
         {isMobileMenuOpen && (<div className="md:hidden glass-strong animate-fade-in animate">
            <div className="container mx-auto px-6 py-6 flex flex-col gap-4">
                {navLinks.map((link,index)=>(
                    <a href={link.href} key={index} className="text-lg text-muted-foreground hover:text-foreground py-2" onClick={()=>setIsMobileMenuOpen(false)}>{link.label}</a>
                ))}
                <a href="#contact">
